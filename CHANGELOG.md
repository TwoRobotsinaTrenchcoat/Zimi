...
- **A captured page's own module loader can find its chunks again (#64).** warc2zim stores references relative, so `src="/_next/app.js"` becomes `src="_next/app.js"`. That is right for the browser and wrong for the page's own JavaScript when it reads the attribute back: Turbopack, which Next.js builds with, identifies a chunk by `script.getAttribute("src")` and strips a leading `/_next/`. With the slash gone every chunk registered under a name nothing was waiting for, the entry module never ran, and the page rendered and did nothing — no error, no failed request. 191 module factories run on the live site and none in the ZIM. Zimi installs a small shim after the replay shell that answers the attribute the way the server did, and draculatheme.com's theme switch and colour tabs work from the ZIM.
- **Zimi's own reader waits for a replayed page to wake up.** Two passes that tidy a captured page — the consent-wall sweep and the JS-driven chrome settle — edit the article's DOM. That is safe on a frozen capture and not on one whose scripts still run: React finding the DOM changed under it during hydration throws and re-renders a stump. The same page worked in a bare iframe and broke in ours, which is the tell. On a live replay both now wait for the page to settle.

- **A capture keeps both of the site's faces, when the site has two.** A site with a dark mode serves a different page to a reader who prefers dark, and a capture could only ever keep one of them — so someone reading in dark opened a captured site and got the light one. Both are kept now, stored as an ordinary entry beside the first so any other viewer can still open it, and the reader shows whichever matches the theme in front of you. Most of the second face's assets are shared with the first, so it costs little; the few it does not share are carried on its own visit, which is what stopped draculatheme.com's light hero from being a hole where a picture should be. Sites that keep their themes entirely in their own hands are covered too — dracula stamps `data-theme="dark"` whatever the browser asks for, so Zimi looks for a control the page itself labels as a theme switch and presses it. Which face is which is measured from what the page paints, not from what was asked for.

- **The home screen orders your ZIMs the way you ask, and defaults to alphabetical (#67).** Article count was the old default; it rewards big files rather than the one you are looking for, and it disagreed with the list in Settings. Order and view now share one control at the top of the library: Alphabetical, Recently added, Recently updated, Most articles. The order applies to every shelf, Favorites and collections included — those are built from stored name lists and used to keep the order things were starred in. Sort by a date and each card shows that date, so the order you asked for is one you can see rather than one you have to trust.
...
- **A capture's own picture no longer libels it (#64).** The picture of the packaged page was taken from the converter's output and the file was patched afterwards — but the patch is what installs the module-loader shim above, so the photograph was of a ZIM still carrying the very defect it fixes: a shell with an empty middle. Then a height comparison against the live page warned that "something did not survive capture" about a file that renders perfectly the moment you open it. Two changes: the picture is taken during the rewrite, from the bytes going into the file, and a replayed page is given time to boot first. Hydration is CPU work, not network work, so "the network went quiet" arrived while the page was still a shell — on draculatheme.com, quiet at 900px and finished at 19,851. The stored dimensions go from 1280x19858, 1280x900 to 1280x19858, 1280x19858, and the warning stops firing.

- **Every PDF opened through the reader was blank (#71).** Reported by Joe (WB3IHY) with the cause and the fix. The reader loads Zimi's bundled pdf.js viewer into the same frame it loads articles into, and that frame's load runs the passes that tidy up a captured web page. At that instant pdf.js has parsed the document but has not painted a canvas — that comes a moment later — so its container is a tall, empty, transparent box with no embedded media in it, which is exactly how the hollow-block sweep recognises an abandoned ad slot. It was collapsed to nothing, and no page ever rendered. Zimi's own pages are now excluded from every pass meant for a capture.

- **The engine offered for an application is no longer the one that breaks it.** The recommendation asked whether the document arrived empty, which catches an obvious shell and misses a server-rendered app entirely: Next.js sends draculatheme.com's whole theme grid as HTML, so by text alone it read as an ordinary page and Fast was offered for something whose palette, theme switch and search are all JavaScript. The probe now also looks for a framework's runtime in the page it fetched, and the preview says which kind of page it found, so the choice is visible rather than inferred.

- **Background jobs stopped erasing each other's work.** Five separate things read the metadata cache, change part of it and write the whole thing back — the shape worker, the provenance walk, the Q-ID pass, and registering or unregistering a ZIM — on different threads, with nothing between them. Whichever saved second was building on a copy taken before the first, so the first's work vanished. A lost shape is not cosmetic: the file gets measured again, and measuring walks every entry in it. On a large library that is the machine grinding every quarter of an hour, for ever. They all go through one lock now, and the Q-ID pass no longer opens the file itself — which also stops it merging into a cache format this build has stopped understanding.

- **A card says which date it is showing.** Two orders put a date on a card and they mean opposite things — the day a ZIM arrived, and the day its contents changed — so it reads "Added 12 Sep" or "Updated 12 Sep" rather than a bare number you have to remember the context of. Written out, not counted down: "21 hours ago" was both longer and less useful than the date it was standing in for, and it pushed the line onto two rows on a phone. A ZIM's own date steps aside while the sorted one is there, so the same fact is never printed twice.

- **The step rail on a running capture stops going through the dots.** It spanned centre to centre, so the line ran under each dot and out the other side; what showed was a line ending halfway into a circle.

- **Changing the library's order moves the cards instead of rebuilding them.** Sorting changes only the sequence — the same cards, in a different place — but the whole shelf was thrown away and made again, which reads as the page flashing and re-fetched every icon on the way. The cards now slide from where they were to where they belong, in one movement, and nothing is re-requested. Honoured `prefers-reduced-motion`: the order changes, the travel does not.

- **The Discover strip loses its scrollbar.** The thumb was a short line reaching about halfway across, under the cards, reading as an artefact rather than a control. The next card is always cut off at the strip's edge, which says "there is more" better than a bar did.

- **The picture of the page you are capturing arrives at the start on every engine.** The fast engine took it at packaging time, so the engine most likely to be chosen for a quick capture was the one that showed nothing while it worked. Same single visit, same cost, taken at the beginning.

- **One unusable reference can no longer end a capture.** Found by the engine matrix before this release: capturing nerdfonts.com with the builtin engine died outright, because the page asks for a font whose filename contains spaces. Every browser fetches that by encoding them; Python refuses it, and the refusal is not the kind of error the asset reader was catching, so it escaped and took the whole run with it. The URL is encoded the way a browser encodes it, so the font is kept rather than lost, and the reader now refuses a bad reference the way its sibling already did — the worst an unusable reference may cost is itself.

- **A cancelled capture stops the browser it started.** Both streaming runners collected the child process only when the command finished normally, and the progress sink they stream through raises on a pending cancel by design — so cancelling a capture skipped both the kill and the reap every time. A NAS accumulated twenty zombie processes in twenty-one hours. The zombies were the cheap part: what preceded each one was a browser tree still reading from a saturated disk for a capture whose result had already been thrown away. Children now start in a process group of their own and the whole group is stopped and collected on every exit path, and the container image gains an init to catch anything still missed.

- **A capture made with the alive or zimit engine now keeps its two pictures too.** 1.9.2 shipped the feature without them, which left the engine most likely to produce a doubtful capture as the one engine that could not show you. warc2zim writes those ZIMs, takes no arbitrary metadata and seals the file — but Zimi already rewrites that file once at creation to add everything else it knows, so the pictures go in with the rest, as metadata, the same way every other engine stores them. The pair of sizes goes in too, so About can compare them here as it does elsewhere.

- **The Create page answers before you press the button, and clears after.** The address was only examined when the field lost focus, so pasting one and pressing Create straight away started the job without it ever being looked at — and that look is what picks the engine, and what notices a video address typed under Web page. Typing now asks after a short pause, and Create waits for an unanswered question before it starts. A finished run also leaves the form empty for the next one instead of holding the last one's address, title and preview — unless you had already started typing, which is kept. And the picture of the live page, which is taken seconds in, is now on screen while the capture runs rather than only afterwards.

- **A place in its own timezone is a place.** The world clock is a curated tour of 28 cities, and a location that shares an offset with none of them lit nothing at all — click Eucla, on the south coast of Western Australia at UTC+8:45, and every card stayed dark, which reads as a click that did nothing. Somewhere off the tour now gets a card of its own, inserted where its clock belongs rather than at the head of the row. Fourteen of the map's 173 dots take that path.
...
- **A library re-render stops costing a disk read per source.** A ZIM's shelf icon was sent with a validator over the icon's own bytes and no freshness window, so the browser asked about every icon on every render — every sort, every view toggle, every return to the page — and answering each ask meant opening the archive and hashing the illustration under the lock every search and every article read also needs. The validator now names the file rather than its contents, so a revalidation is answered before the lock is taken, and there is a real freshness window. On a 74-source library, four page loads and six view toggles: 74 archive reads in total, all on the first paint, none after; icons draw in about 85 ms on a revisit against 390 ms cold. The home screen also stops rebuilding itself three times while the library, the collections and the provenance walk each arrive.

- **Discover stops holding the whole library up while it picks.** The random-article endpoint took the global archive lock once and held it across every attempt — up to fifty for a Wiktionary card — and ran all of them even when the first pick was already good, because the judging happened after the loop. Discover fires one of those per card, in parallel, on page load. It is now one lock acquisition per attempt, and each pick is judged as it arrives, so the usual case reads once.

- **Manage → Creator opens now instead of in four seconds.** Two costs, both paid once per process, which sounds fine until you notice a deploy restarts the process. Finding out whether the rendered engine works means launching a browser — 2.5 s — and the pane waited for it; it is probed on a thread now, so the pane answers in 21 ms and fills the capability rows in when the answer lands. And "which ZIMs did Zimi make" opened every archive in the library and forgot the answer on exit; it is remembered on disk and warmed in the background at startup, so a 73-ZIM library no longer pays 73 archive opens after every restart.
...
- **Every capture keeps two pictures.** The live page as the web served it, and the same page as this ZIM serves it, both full page, both taken under the same treatment, stored as metadata beside the illustration (not as entries) and shown side by side on the Create card and in About. The fast engine takes them too when a browser is installed. When the packaged page comes out a fraction of the live one's height, the job log says so: not a score, a collapse detector.
- A page's `background=` picture (body, table, cell — spacejam.com/1996 tiles its starfield that way) is now carried by both engines; before, the packaged page was black where the live one had stars. The first defect the two pictures caught.
- A ZIM under a megabyte no longer shows as 0 B on its card; sizes carry 1 KB resolution.
