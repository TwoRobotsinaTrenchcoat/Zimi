...
- **Two background jobs stopped erasing each other's work.** The shape worker and the provenance walk both read the metadata cache, change part of it and write the whole thing back, on different threads. Whichever saved second was building on a copy taken before the first, so the first's work vanished — and a lost shape means the file gets measured again, which walks every entry in it under the lock that every search needs. On a large library that is minutes of the machine grinding, every quarter of an hour, for ever. All four writers now take one lock.
